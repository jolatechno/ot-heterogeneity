...
def KL_divergence(array_A: _np.array, array_B: _np.array, min_value_avoid_zeros: float=1e-16):
	...
	Parameters:
		array_A (np.array): 2d-array of shape (`num_dimension`, `size`) or 1d vector of legth `num_dimension`
		array_B (np.array): 2d-array of shape (`num_dimension`, `size`) or 1d vector of legth `num_dimension`
	...
	if is_output_1d:
		...
		size     = array_A.shape[1] if not is_array_A_1d else array_B.shape[1]
		array_A_ = array_A if not is_array_A_1d else _np.repeat(_np.expand_dims(array_A, axis=1), size, axis=1)
		array_B_ = array_B if not is_array_B_1d else _np.repeat(_np.expand_dims(array_B, axis=1), size, axis=1)
		...
		return _np.sum(array_A_ * _np.log2(_np.maximum(array_A_, min_value_avoid_zeros) / _np.maximum(array_B_, min_value_avoid_zeros)), axis=0)
...
def total_variation(array_A : _np.array, array_B : _np.array):
	...
	if is_output_1d:
		...
		return _np.sum(_np.abs(array_A_ - array_B_, minimum), axis=0)